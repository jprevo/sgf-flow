...
jest.mock('../services/sgf-directory.service', () => {
  ...
  return {
    ...actual,
    addSgfDirectory: (dirPath: string) => actual.addSgfDirectory(dirPath, testConfigPath),
    removeSgfDirectory: (dirPath: string) => actual.removeSgfDirectory(dirPath, testConfigPath),
...
describe('SGF Directory Controller', () => {
  ...
  describe('POST /api/sgf-directories', () => {
    it('should add a new directory', async () => {
      ...
      expect(response.body).toHaveProperty('message', 'Directory added successfully');
      expect(response.body).toHaveProperty('directories');
    ...
    it('should return 409 if adding parent when subdirectory exists', async () => {
      await request(app).post('/api/sgf-directories').send({ path: testSubDir });
  ...
  describe('DELETE /api/sgf-directories', () => {
    it('should remove a directory', async () => {
      ...
      expect(response.status).toBe(200);
      expect(response.body).toHaveProperty('message', 'Directory removed successfully');
    ...
    it('should return 404 if directory not in config', async () => {
      ...
      expect(response.status).toBe(404);
      expect(response.body).toHaveProperty('error', 'Failed to remove directory');
