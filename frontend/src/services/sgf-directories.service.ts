...
export async function getSgfDirectories(): Promise<string[]> {
  const response =
    await httpClient.get<SgfDirectoriesResponse>("/sgf-directories");
  return response.data.directories;
}

export async function addSgfDirectory(path: string): Promise<string[]> {
  const response = await httpClient.post<SgfDirectoriesResponse>(
    "/sgf-directories",
    { path },
  );
...
export async function removeSgfDirectory(path: string): Promise<string[]> {
  const response = await httpClient.delete<SgfDirectoriesResponse>(
    "/sgf-directories",
    {
      data: { path },
    },
